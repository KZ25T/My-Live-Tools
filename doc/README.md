...
- 启动本操作系统。
- 启动 GParted （命令：`sudo gparted`）给硬盘分区，给安装本系统划分出硬盘空间（至少一个 ext4 格式作为根目录分区），并挂载到 `/mnt/debian` 上。（当然挂载点随意）
  ...
  - 举例：如果你的分区调整为：
    ...
    - 挂载 ESP 分区： `sudo mkdir -p /mnt/debian/boot/efi && sudo mount /dev/sda2 /mnt/debian/boot/efi`
...
- 卸载系统：`sudo umount /mnt/debian/boot/efi && sudo umount /mnt/debian`
